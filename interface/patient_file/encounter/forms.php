...
<style type="text/css">@import url(<?php echo $GLOBALS['webroot'] ?>/library/ESign/css/esign.css);</style>
<?php 
$esignApi = new Api();
?>
...
$(document).on( 'click', '.esign-amendment-link', function( e ) {
	e.preventDefault();
	$(this).parents('tr').next('.esign-log-amendment-row').toggle();
}); 
...
<?php
  if ($result = getFormByEncounter($pid, $encounter, "id, date, form_id, form_name, formdir, user, deleted")) {
    ...
    foreach ($result as $iter) {
        ...
        if ( $esign->isLocked() ) {
            echo "<a href=# class='css_button_small form-edit-button-locked' id='form-edit-button-".$formdir."-".$iter['id']."'><span>".xl('Locked')."</span></a>";
        } else {
            echo "<a class='css_button_small form-edit-button' id='form-edit-button-".$formdir."-".$iter['id']."' target='".
