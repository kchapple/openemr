...
{   
    ...
    public function getSignatures()
    {
        ...
        $statement = "SELECT E.id, E.tid, E.table, E.uid, U.fname, U.lname, E.datetime, E.is_lock, E.amendment, E.hash FROM esign_signatures E ";
        ...
        while ( $row = sqlFetchArray( $result ) ) {
            $signature = new Signature( $row['id'], $row['is_lock'], $row['uid'], $row['fname'], $row['lname'], $row['datetime'], $row['hash'], $row['amendment'] );
    ...
    protected function getLastLockHash()
    {
        ...
        $row = sqlQuery( $statement, array( $this->_tableId, $this->_tableName, Signature::ESIGN_LOCK ) );
        $hash = '';
    ...
    public function isLocked()
    {
        ...
        $statement .= "ORDER BY E.datetime DESC LIMIT 1 ";
        $row = sqlQuery( $statement, array( $this->_tableId, $this->_tableName, Signature::ESIGN_LOCK ) );
        if ( $row && $row['is_lock'] == Signature::ESIGN_LOCK ) {
            return true;
        }
    ...
    {
        $statement = "INSERT INTO `esign_signatures` ( `tid`, `table`, `uid`, `datetime`, `is_lock`, `hash`, `amendment` ) ";
        $statement .= "VALUES ( ?, ?, ?, NOW(), ?, ?, ? ) ";
        
        // Make type string
        $isLock = Signature::ESIGN_NOLOCK;
        if ( $lock ) {
            $isLock = Signature::ESIGN_LOCK;
        }
